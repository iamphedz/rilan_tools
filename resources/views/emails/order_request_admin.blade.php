...
@endcomponent
@endslot
{{ $order_request->name }} has send an order request. <br /><br />Please process this immediately. Thank you.
...
<div>
        ...
        <tr>
            <td colspan="5">
                <p style="font-size: 90%;"><br />
                    ...
                    Ordered on {{ $Carbon::parse($order_request->created_at)->format("F j, Y h:m:s a") }}
                    <br />
                    <br />
    ...
    <table>
        ...
        <tr class="text-right">
            <td class="text-left" width="25%">{{ $item['name'] }} ({{ $Brand::find($Product::find($item['id'])->brand_id)->brand_name }})</td>
